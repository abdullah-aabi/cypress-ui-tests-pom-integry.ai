  ...
  it("The user is able to create a new account.", () => {
    cy.readFile("src/utils/user-creds.json").then((creds) => {
      ...
      */ // Removed as per new updates
      cy.get(SignUpLocators.signupPopup).should('be.visible')
      cy.get(SignUpLocators.signupCLoseBtn).should('be.visible')
      cy.get(SignUpLocators.popupCloseBtn).click({ force: true });
      cy.isUserLoggedIn();
    });
  });
});
